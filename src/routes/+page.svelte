<script lang="ts">
    ...
    import {togglePrepare, state, toggleStart, TimerState, time} from "$lib/timer";
...
<div class="flex flex-col justify-center items-center mt-32 mx-auto w-3/4 relative">
    {#key $sequence}
        <p in:fade={{duration: 450}} class:opacity-0={$state !== TimerState.IDLE}
...
<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
    <p class:text-black={$state === TimerState.IDLE || $state === TimerState.RUNNING}
       class:text-red-500={$state === TimerState.PREPARING}
       class:text-green-500={$state === TimerState.PREPARED}
       class="text-6xl font-bold transition-all duration-300 ease-in-out" class:scale-150={$state === TimerState.RUNNING}
    >
